...
_Определение._ Оператор $$A$$ является *сжимающим оператором (сжатием)* на $$D$$, если существует $$q \in (0,1)$$ такое, что для любых $$x_1, x_2 \in D$$
...
Отсюда следует, что если $$x_*$$~--- простой корень и $$f$$ имеет 2-ю непрерывную производную (в этом случае $$\varphi'(x)$$ непрерывна), то $$|\varphi'(x)| < 1$$ в окрестности корня $$x_*$$. Поэтому при выборе начального приближения в указанной окрестности метод Ньютона сходится.
Если всюду $$|ff''| < (f')^2$$, то итерации сходятся при произвольном нулевом приближении.
...
{\bf Теорема.} [Самарский, Гулин, 1989] Пусть $$x_*$$~--- простой вещественный корень уравнения (eq1) и пусть $$f'(x) \neq 0$$ в окрестности
$$B = \{x \colon |x - x_*| < r\}$$.
...
имеет <span>*метод Ньютона с параметром* </span> $$\label{newton2}
f'(x_k)\frac{x_{k+1} - x_k}{p} + f(x_k) = 0.$$

<span>**Теорема.**</span> [@SamGul] Пусть $x_*$ — корень кратности $p$
...
среднего значения с обобщенной форме [@IlPoz с. 350].
...
\frac{(x_k - x_*)^{p+1}}{p(p+1)}.$$ Следовательно,
...
представления: $$\label{pr1}
f(x) \approx a(x - x_*)^p, \quad f'(x) \approx pa(x-x_*)^{p-1}.$$
Обозначим отклонение от корня $\Delta_k = x_k - x_*$.
$$x_{k+1} = x_k - \frac{f(x_k)}{f'(x_k)},$$
$$x_{k+1} - x_* = x_k - x_* - \frac{f(x_k)}{f'(x_k)}.$$ Подставим
представление (\[pr1\]):
$$x_{k+1} - x_* \approx x_k - x_* - \frac{1}{p}(x_k - x_*),$$ отсюда
$$\Delta_{k+1} \approx \left(1 - \frac{1}{p}\right)\Delta_k.$$

Знаменатель геометрической прогрессии можно приближенно определить из
следующего соотношения: $$\label{znam}
q \approx q_k \equiv \frac{x_{k+1} - x_k}{x_k - x_{k-1}}.$$ Когда
итерации приближаются к корню, $q_k \to q$. Если мы визуально наблюдаем
...
величина $|x_{k+1}|$ в правой части поставлена для того, чтобы
...
$$\frac{q}{1-q}(x_{k+1} - x_k) < \varepsilon|x_{k+1}|,$$ где
$\varepsilon$ — требуемая относительная точность, а $q$ вычисляется по
...
$$\label{usl1}
f'(x) > 0, f''(x) > 0,$$ либо $$\label{usl2}
f'(x) < 0, f''(x) < 0.$$ Тогда последовательность $\{x_k\}$,
определенная согласно методу Ньютона с $x_0 = b$, монотонно убывает и
сходится к $x_*$.

<span>**Теорема.** </span> Пусть для всех $x \in [a,b]$ либо
$$f'(x) > 0, f''(x) < 0,$$ либо $$f'(x) < 0, f''(x) > 0.$$ Тогда
последовательность $\{x_k\}$, определенная согласно методу Ньютона с
...
докажем, что тогда $$\label{d1}
x_* < x_{k+1} < x_k.$$ Перепишем уравнение метода Ньютона в виде
$$x_k - x_{k+1} = \frac{f(x_k) - f(x_*)}{f'(x_k)}$$ и воспользуемся
формулой конечных приращений Лагранжа. Тогда получим $$\label{p1}
x_k - x_{k+1} = \frac{(x_k - x_*)f'(\xi_k)}{f'(x_k)},$$ где
$\xi_k \in (x_*, x_k)$. Пусть выполнено условие (\[usl1\]). Тогда
...
<span>**Теорема.** </span> [@SamGul] Предположим, что в
...
силу непрерывности $G'$: $$\|G(x_* + d^k) - G(x_*) - G'(x^k)d^k\|_Y =
